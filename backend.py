...
def main():
    ...
    for x in transactionList:  # goes through a list of transactions
        ...
        toaccountNum = eachTransaction[1]
        ...
        if transCode != "NEW" and transCode != "EOS":  #   Checks if account exists or not
            if fromAccount == "0000000" and toaccountNum == "0000000" :
                ...
                exit("Fatal ERROR")
        if transCode == "DEP":
            deposit(masterAccountDict, toaccountNum, amount)
        elif transCode == "WDR":
            withdraw(masterAccountDict, fromAccount, amount)
        elif transCode == "XFR":
            transfer(masterAccountDict, toaccountNum, fromAccount, amount)
        elif transCode == "NEW":
            createAccount(masterAccountDict, fromAccount, accountName)
        elif transCode == "DEL":
            deleteAccount(masterAccountDict, fromAccount, accountName)
